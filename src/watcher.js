...
function isNewState(type, value, store) {
  return (store[type] === undefined ||
          (value !== null && store[type] !== null &&
                   typeof value === 'object' &&
...
export default function createWatcher(model, config) {
  const state = {
    ...
    id : 0,
    listeners : {subscribe : [], block : [], poll : [], error : []},
    handler : null,
  ...
  function setupWebSocket() {
    if (reWsEndpoint.test(state.config.rpcUrl)) {
      ...
      state.ws.onopen = () => {
        ...
        if (state.handler)
          throw new Error('Existing poll setTimeout handler set');
        if (state.watching) {
          poll.call({
            state,
            interval : 0,
      ...
      state.ws.onclose = err => {
        ...
        log(`Reconnecting in ${
            state.config.wsReconnectTimeout / 1000} seconds.`);
  ...
  state.initialFetchPromise =
      new Promise(resolve => { state.initialFetchResolver = resolve; });

  function subscribe(listener, id, batch = false) {
    if (!isEmpty(state.storeTransformed)) {
      const events =
          Object.entries(state.storeTransformed)
              .map(([ type, value ]) =>
                       ({type, value, args : state.keyToArgMap[type] || []}));
  ...
  function alertListeners(events) {
    if (!isEmpty(events))
      state.listeners.subscribe.forEach(({listener, batch}) =>
                                            batch ? listener(events)
                                                  : events.forEach(listener));
  }

  function poll() {
    const interval = this.interval !== undefined ? this.interval
                                                 : this.state.config.interval;
    log('poll() called, %s%s', 'interval: ' + interval,
    ...
    this.state.handler = setTimeout(async () => {
      try {
        ...
        if (this.state.cancelPromiseId === promiseId)
          return;

        if (typeof this.resolveFetchPromise === 'function')
          this.resolveFetchPromise();

        if (this.state.latestBlockNumber !== null &&
            blockNumber < this.state.latestBlockNumber) {
          // Retry if blockNumber is lower than latestBlockNumber
          log(`Stale block returned, retrying in ${
              this.state.config.staleBlockRetryWait / 1000} seconds`);
          poll.call({
            state : this.state,
          ...
          if (this.state.latestBlockNumber === null ||
            ...
            state.listeners.block.forEach(
                ({listener}) => listener(this.state.latestBlockNumber));
          }
          const events = Object.entries(data)
                             .filter(([ type, value ]) => isNewState(
                                         type, value, this.state.store))
                             .map(([ type ]) => ({
                                    type,
        ...
        state.listeners.error.forEach(({listener}) =>
                                          listener(err, this.state));
        if (!this.state.handler)
          return;
        // Retry on error
        log(`Error occured, retrying in ${
            this.state.config.errorRetryWait / 1000} seconds`);
        poll.call({
          state : this.state,
  ...
  const watcher = {
      ...
      return {
        unsub() {
          state.listeners.subscribe =
              state.listeners.subscribe.filter(({id : _id}) => _id !== id);
        }
      };
    },
    batch() {
      ...
      return {
        unsub() {
          state.listeners.block =
              state.listeners.block.filter(({id : _id}) => _id !== id);
        }
      };
    },
    onPoll(listener) {
      ...
      return {
        unsub() {
          state.listeners.poll =
              state.listeners.poll.filter(({id : _id}) => _id !== id);
        }
      };
    },
    onError(listener) {
      ...
      return {
        unsub() {
          state.listeners.error =
              state.listeners.error.filter(({id : _id}) => _id !== id);
        }
      };
    },
    start() {
    ...
    awaitInitialFetch() { return state.initialFetchPromise; },
    get initialFetch() { return state.initialFetchPromise; },
    get schemas() { return state.model; }
  };

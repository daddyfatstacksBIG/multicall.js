...
export function _makeMulticallData(calls) {
  const values = [ calls.map(
      ({
        ...
        args,
        returnTypes
      }) => [target, keccak256(method).substr(0, 10) +
                         (args && args.length > 0
                              ? strip0x(encodeParameters(args.map(a => a[1]),
                                                         args.map(a => a[0])))
                              : '')]) ];
  const calldata =
      encodeParameters([ {
...
const makeMulticallData =
    memoize(_makeMulticallData, (...args) => JSON.stringify(args));
...
export default async function aggregate(calls, config) {
  ...
  calls = calls.map(({call, target, returns}) => {
    ...
    const [argTypesString, returnTypesString] =
        method.match(INSIDE_EVERY_PARENTHESES).map(match => match.slice(1, -1));
    const argTypes = argTypesString.split(',').filter(e => !!e);
    invariant(argTypes.length === argValues.length,
  ...
  const returnTypeArray = calls.map(({returnTypes}) => returnTypes)
                              .reduce((acc, ele) => acc.concat(ele), []);
  const returnDataMeta = calls.map(({returns}) => returns)
                             .reduce((acc, ele) => acc.concat(ele), []);

  invariant(returnTypeArray.length === returnDataMeta.length,
            'Missing data needed to parse results');

  const outerResultsDecoded =
      decodeParameters([ 'uint256', 'bytes[]' ], outerResults);
  ...
  const parsedVals = outerResultsDecoded.reduce((acc, r) => {
    r.forEach((results, idx) => {
      ...
      acc.push(...resultsDecoded.map((r, idx) => {
        if (types[idx] === 'bool')

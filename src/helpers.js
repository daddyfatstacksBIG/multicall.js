...
export function strip0x(str) { return str.replace(/^0x/, ''); }

export function typesLength(types) { return types.length; }
...
export function padLeft(string, chars, sign) {
  ...
  return ((hasPrefix ? '0x' : '') + new Array(padding).join(sign ? sign : '0') +
          string);
}
  ...
  if (ws) {
    ...
    return new Promise((resolve, reject) => {
      ws.send(JSON.stringify({
        ...
        method : 'eth_call',
        params : [
          {to : multicallAddress, data : abiEncodedData}, block || 'latest'
        ],
        id
      }));
    ...
    return web3.eth.call({to : multicallAddress, data : abiEncodedData});
  } else {
